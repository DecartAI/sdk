...
import { WEATHER_OPTIONS, WeatherConditions } from "./components/WeatherConditions";
...
function App() {
  ...
  const generateOutfit = async () => {
    ...
    try {
      ...
      setResultFile(new File([resultBlob], "result.webp", { type: "image/webp" }));
    } catch (error) {
      console.error(error);
    } finally {
      setIsLoading(false);
  ...
  return (
    <div className="app">
      ...
      <div className="layout">
        <div>
          <WeatherConditions value={condition} onChange={setCondition} options={WEATHER_OPTIONS} />

          <div className="file-input">
            <label>
              Image:
              <input type="file" accept="image/*" onChange={handleFileChange} className="file-input-control" />
            </label>
          </div>

          <img src={imageUrl} alt="Selected outfit" className="preview-image" />

          <button type="button" onClick={generateOutfit} disabled={isLoading || !imageFile}>
        ...
        <div>
          ...
          {resultUrl ? (
            ...
            <p className="result-placeholder">{isLoading ? "Generating..." : "No result yet."}</p>
          )}
        </div>
      </div>
    </div>
