...
test("e2e: LiveKit control flow through proxy", { timeout: 30_000 }, async (t) => {
  if (!DECART_API_KEY) {
  ...
  // livekit_join → livekit_room_info
  ws.send(JSON.stringify({ type: "livekit_join" }));
  assert.equal((await waitFor("livekit_room_info")).type, "livekit_room_info");
  console.log("  ✓ livekit_room_info");
