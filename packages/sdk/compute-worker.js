...
function buildDatasets(runs) {
  ...
  runs.forEach((run) => {
    ...
    datasets.bitrateTx.push({
      label,
      ...lineBase,
      data: points.map((p) => ({ x: p.x, y: p.d.senderBitrateBps != null ? p.d.senderBitrateBps / 1000 : null })),
    });
    datasets.bitrateRx.push({
      label,
      ...lineBase,
      data: points.map((p) => ({ x: p.x, y: p.d.receiverBitrateBps != null ? p.d.receiverBitrateBps / 1000 : null })),
    });
...
const CSV_HEADER = [
  ...
  "keyFramesInTotal",
  "keyFramesInDelta",
  "framesDroppedInTotal",
  "framesDroppedInDelta",
  "freezeCountTotal",
  "freezeCountDelta",
  "totalFreezesDurationSec",
  "pauseCountTotal",
  "pauseCountDelta",
  "qlReasonOut",
  "ocrLatencyMs",
