import { z } from "zod";
import type { LiveKitManager } from "./livekit-manager";
...
export const realtimeMethods = (
  ...
  const assertConnected = () => {
    const state = livekitManager.getConnectionState();
  ...
  const set = async (input: SetInput): Promise<void> => {
    ...
    await livekitManager.setImage(imageBase64, { prompt, enhance, timeout: UPDATE_TIMEOUT_MS });
  };
  ...
  const setPrompt = async (prompt: string, { enhance }: { enhance?: boolean } = {}): Promise<void> => {
    ...
    const emitter = livekitManager.getWebsocketMessageEmitter();
    ...
    try {
      ...
      const sent = livekitManager.sendMessage({
        type: "prompt",
